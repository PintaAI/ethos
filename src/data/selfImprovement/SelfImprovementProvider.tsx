...
type SelfImprovementContextValue = {
  today: string;
  habits: Habit[];
  habitLogs: HabitLog[];
  timeBoxes: TimeBox[];
...
const SelfImprovementContext = createContext<SelfImprovementContextValue | null>(null);

export function SelfImprovementProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const timeout = setTimeout(() => {
      void refresh(true)
        .catch((error) => console.warn("Failed to load self-improvement data", error))
  ...
  useEffect(() => {
    const subscription = AppState.addEventListener("change", (state) => {
      ...
      void refresh().catch((error) => console.warn("Failed to refresh self-improvement data", error));
    });
  ...
  const value: SelfImprovementContextValue = {
    today,
    habits,
    habitLogs,
    timeBoxes,
  ...
  return <SelfImprovementContext value={value}>{children}</SelfImprovementContext>;
}

export function useSelfImprovement() {
  const value = use(SelfImprovementContext);
  if (!value) throw new Error("useSelfImprovement must be used within SelfImprovementProvider");
  return value;
}
