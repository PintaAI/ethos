...
import { TIME_BOX_COLORS } from "@/components/selfImprovement/TimeMapDial";
...
export default function GrowthSetup() {
  ...
  const growth = useSelfImprovement();
  ...
  async function finish(skip = false) {
    ...
    try {
      ...
      Alert.alert(t("growthSetup.saveErrorTitle"), error instanceof Error ? error.message : t("growthSetup.saveError"));
        ...
        <View className="gap-6 rounded-[32px] border p-4" style={{ backgroundColor: surface, borderColor }}>
          ...
          <View className="gap-3">
            ...
            {Platform.OS === "ios" ? (
                ...
                {name.length === 0 ? (
                  <View pointerEvents="none" className="absolute inset-0 justify-center px-4">
                    <Text className="text-base" style={{ color: appTheme.colors.muted }}>
                      {t("growthSetup.habitPlaceholder")}
                    </Text>
                  </View>
                ) : null}
                <TextInput
                  ...
                  accessibilityLabel={t("growthSetup.habitName")}
              ...
              <TextInput
                ...
                onChangeText={setName}
                accessibilityLabel={t("growthSetup.habitName")}
                placeholder={t("growthSetup.habitPlaceholder")}
          ...
          <View className="gap-3">
            <Text className="text-xs font-bold uppercase tracking-[2px]" style={{ color: appTheme.colors.muted }}>
              {t("growthSetup.tinyHabit")}
            </Text>
            <View className="flex-row flex-wrap gap-2">
              {habitDurations.map((minutes) => {
                const selected = preferredDuration === minutes;
                const label = minutes === 1
                  ? t("growthSetup.durationMinute")
                  : minutes === 60
                    ? t("growthSetup.durationHour")
                    : t("growthSetup.durationMinutes", { minutes });
                return (
                  <GlassBox
                    key={minutes}
                    isInteractive
              ...
              {TIME_BOX_COLORS.map((item) => {
                ...
                return (
                  ...
                  >
                    <Pressable
                      accessibilityRole="radio"
                      accessibilityLabel={t("growthSetup.chooseColor", { color: item })}
                      accessibilityState={{ selected, disabled: growth.loading }}
                      disabled={growth.loading}
          ...
          {valid ? (
            <View className="flex-row items-center gap-3 rounded-3xl p-4" style={{ backgroundColor: alpha(color, 0.15) }}>
              ...
              <Text className="min-w-0 flex-1 font-bold leading-5" style={{ color: appTheme.colors.foreground }}>
                {t("growthSetup.commitment", {
                  rhythm: t(`growthSetup.recurrence.${recurrence}`),
                  habit: name.trim(),
                  duration: preferredDuration === 1
                    ? t("growthSetup.durationMinute")
                    : preferredDuration === 60
                      ? t("growthSetup.durationHour")
                      : t("growthSetup.durationMinutes", { minutes: preferredDuration }),
                })}
              </Text>
            </View>
          ) : null}
          ...
          >
            ...
            >
              <Text className="font-bold" style={{ color: appTheme.colors.primary }}>
                {editing === null ? t("growthSetup.addAnother") : t("common.save")}
              </Text>
            </Pressable>
          </GlassBox>
        </View>
        ...
        >
          ...
          >
            {savingAction === "skip" ? (
              ...
              <Text className="font-bold" style={{ color: appTheme.colors.muted }}>{t("growthSetup.skip")}</Text>
            )}
          </Pressable>
        </GlassBox>
        <GlassBox
        ...
        >
          ...
          >
            {savingAction === "finish" ? (
              ...
              <Text className="font-bold" style={{ color: appTheme.colors.inverseForeground }}>{t("growthSetup.finish")}</Text>
            )}
          </Pressable>
        </GlassBox>
      </View>
